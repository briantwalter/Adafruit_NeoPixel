...
//
// cylon_3po_wbutton           Cylon-3PO light display box with button 
// version                     0.0.1
// author                      Brian Walter @briantwalter
// description                 Custom built light bar with arcade button
//                             to use NeoPixel RGB LEDs powered by Arduino

// configuration constants
const int button = 2;        	   // button on digital 2
const int noofpixels = 21;         // number of NeoPixels in the light bar
...
void setup() {
  ...
  pinMode(button, INPUT);  	// set up button input
  //Serial.begin(9600);           // for debugging to serial
}

// runs constant
void loop() {
  buttonval = digitalRead(button);
  //Serial.print("DEBUG: buttonval is ");
  ...
  if (buttonval == 1) {
    ...
    delay(10);
    if ( buttonval == 1) {
      rainbow(40);  // if someone is holding down the button do rainbow
      colorWipe(strip.Color(0, 0, 0), 50); // off
    }
  }
